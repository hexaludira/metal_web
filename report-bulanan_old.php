...
<section class="content">
   <div class="col-sm-12">
      <form class="form-horizontal" method="POST" action="report-bulanan">
         <div class="col-sm-5">
         ...
         <div id="dvData">
            ...
            <table class="table table-bordered">
               ...
               <tr>
                  ...
                  $q = mysqli_query($con,"SELECT pass,date,no_cell, DAY(dates) as tgl1 FROM data_daily WHERE   day(date)=$tgl and month(date)='$bulan' and year(date)='$tahun' and no_cell=$c GROUP BY date");
                  $tds = mysqli_fetch_array($q);
                  $tgl2 = $tds['tgl1'];
                  if ($tgl==$tgl2) { ?>
               ...
               <tr>
                  ...
                  <?php
                  for ($tgl=1; $tgl<=$hari; $tgl++) {
                  $q = mysqli_query($con,"SELECT detect,date,no_cell, DAY(dates) as tgl1 FROM data_daily WHERE   day(date)=$tgl and month(date)='$bulan' and year(date)='$tahun' and no_cell=$c GROUP BY date");
                  $tds = mysqli_fetch_array($q);
                  $tgl2 = $tds['tgl1'];
                  if ($tgl==$tgl2) { ?>
               ...
               <tr>
                  ...
                  <?php
                  for ($tgl=1; $tgl<=$hari; $tgl++) {
                  $q = mysqli_query($con,"SELECT error,date,no_cell, DAY(dates) as tgl1 FROM data_daily WHERE   day(date)=$tgl and month(date)='$bulan' and year(date)='$tahun' and no_cell=$c GROUP BY date");
                  $tds = mysqli_fetch_array($q);
                  $tgl2 = $tds['tgl1'];
                  if ($tgl==$tgl2) { ?>
